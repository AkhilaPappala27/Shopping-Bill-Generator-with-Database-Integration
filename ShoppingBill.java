...
class Product {
    ...
    Product(String id, String pname, int qty, double price, double totalPrice) {
        ...
        this.purchaseDate = new java.sql.Date(System.currentTimeMillis()); // Current date
    }
    ...
    public static void displayFormat(FileWriter writer) throws IOException {
        String header = "   ------------------------------------------------------------\n" +
                        ...
                        "   ------------------------------------------------------------\n";
        String centeredHeader = centerText(header, 80); // Adjust width as needed
        System.out.print(centeredHeader); // Print to console
        writer.write(centeredHeader);     // Write to file
    }
    ...
    public void display(FileWriter writer) throws IOException {
        ...
        System.out.print(productLine); // Print to console
        writer.write(productLine);    // Write to file 
    }}
    ...
    public static void main(String args[]) {
        ...
        try (FileWriter writer = new FileWriter("invoice_output.txt");
            ...
            String header = centerText("--------------------Invoice-----------------", 80) + "\n" +
                            centerText("Metro Mart Grocery Shop", 80) + "\n" +
                            centerText("3/98 Mecrobertganj New Mumbai", 80) + "\n" +
                            centerText("Opposite Metro Walk", 80) + "\n" +
                            centerText("GSTIN: 03AWBPP8756K592 Contact: (+91) 9988776655", 80) + "\n";
            ...
            String dateInfo = centerText("Date: " + formatter.format(currentDate) + "  " + days[calendar.get(Calendar.DAY_OF_WEEK) - 1], 80) + "\n" +
                              centerText("Contact: (+91) 9998887770", 80) + "\n";
            ...
            do {
                ...
                // Check if product ID exists in the items table
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(query)) {
                    ...
                    if (rs.next()) {
                        // Product found, get name and rate from items table
                        productName = rs.getString("pname");
                        price = rs.getDouble("price");
                        System.out.println("Product Name : " + productName +"\n"+ "Price (per unit) : " + price);
                    } else {
                        // Product not found, ask user for name and rate
